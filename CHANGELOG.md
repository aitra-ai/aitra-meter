...
- **SGLang inference provider** — dedicated `sglang` provider reading `sglang_generation_tokens_total` / `sglang_num_running_reqs` (v0.5.4+ naming) with automatic fallback to the legacy `sglang:` prefix. Requires `--enable-metrics` on the SGLang server. (#41, #52)
- **Triton inference provider** — dedicated `triton` provider for NVIDIA Triton Inference Server. Token counts are approximate (`nv_inference_request_success` × configurable `avg_output_tokens_per_request`, summed across models); the limitation is documented in the provider and guide. (#41, #52)
- **Inference engine auto-discovery** — `--inference-provider=auto` / `inferenceProvider.type: auto` detects the engine from pods on the agent's node via the `aitra-ai.github.io/inference-provider` annotation or `app` label (priority: annotation → app label → generic-prometheus fallback for unknown annotation values), builds the metrics endpoint from the pod IP, and logs provider + token metric per discovered pod at startup. (#41)
- **vLLM latency correlation** — optional read-only scrape of `vllm:time_to_first_token_seconds` / `vllm:time_per_output_token_seconds` histogram totals, logged per energy window at debug level; not re-exposed as Aitra metrics. (#41)
- **`examples/inference-servers/`** — ready-made Helm values per engine (vLLM, SGLang, Triton, TGI, Ollama, auto-discovery) and a new `docs/guides/inference-providers.md` guide covering discovery rules, required server flags, and known limitations, including a TGI maintenance-mode (2025-12-11) deprecation notice. (#41, #52)
...
### Fixed
- Helm `inferenceProvider.config` values now actually reach the measurement agent: every config key is rendered as an `INFERENCE_CONFIG_<KEY>` environment variable and read by the agent (previously only the endpoint was passed, via `INFERENCE_ENDPOINT`, and keys such as `avg_output_tokens_per_request` or the `generic-prometheus` metric names were silently ignored; `INFERENCE_ENDPOINT` is still honoured for backwards compatibility). With `inferenceProvider.type: auto` the chart no longer injects the default static endpoint, and the agent prefers the discovered pod-IP endpoint — previously the values.yaml default `http://localhost:8000/metrics` overrode discovery and broke auto mode. (#41)
