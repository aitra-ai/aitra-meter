...
Deploy the measurement agent as a Kubernetes DaemonSet with `hostPID: true` and `privileged: true`, scheduled only to nodes labeled `aitra-ai.github.io/gpu=true`.
...
- Node affinity via `aitra-ai.github.io/gpu=true` label prevents the agent from scheduling to non-GPU nodes, eliminating wasted pods.
...
- Operators must label GPU-bearing nodes with `aitra-ai.github.io/gpu=true` at cluster setup. This is a one-time administrative step, documented in the Helm chart README.
